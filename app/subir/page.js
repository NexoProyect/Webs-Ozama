...
export default function Subir() {
  ...
  async function enviar() {
    ...
    try {
      ...
      setEstado({ tipo: "ok", texto: `¡Listo, ${data.nombre}! Tu proyecto está publicado.`, url: data.url });
    } catch (e) {
      ...
      <div className="card" style={{ maxWidth: 560 }}>
        {estado && (
          <div className={`msg ${estado.tipo}`}>
            {estado.texto}
            {estado.url && (
              <> <a href={estado.url} target="_blank" rel="noreferrer" style={{ color: "inherit", textDecoration: "underline" }}>Abrir →</a></>
            )}
          </div>
        )}
